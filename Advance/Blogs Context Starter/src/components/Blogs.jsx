...
const Blogs = () => {
  ...
  const {posts, loading} = useContext(AppContext)
    ...
    return (
    <div className='w-11/12 max-w-[675px] py-8 flex flex-col gap-y-7 mt-[70px] mb-[66px]'>
    {
        loading ? 
            ...
            <div key={post.id}>
              ...
              <div className='flex gap-x-3'>
                {post.tags.map( (tag,index) => {
                  return <span key={index} className='text-blue-600 font-bold text-xs mt-[5px]'>{`#${tag}`}</span>
                })}
              </div>
            </div>
          )))
